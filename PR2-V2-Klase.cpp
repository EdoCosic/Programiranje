...
char* AlocirajIKopiraj(const char* tekst)
{
    ...
    char* noviTekst = new char;
    strcpy_s(noviTekst, velic, tekst);
    return noviTekst;
}
...
public:
   ...
   /*flt. ctor
    ...
    Sjediste()
    {
        _sjedisteId = AlocirajIKopiraj(GetRandomString(10));
        _red = ' ';
        _kolona = 0;
    }
    /*   Z2.2
...
public:
    ...
    Dvorana(const char* naziv, int brojRedova, int brojKolona)
    {
        ...
        for (int i = 0; i < _brojRedova; i++) 
        {
            ...
            {
                _sjedista[i][j].SetRed((char)(65 + i));
    ...
    Sjediste* GetSjediste(char red, int kolona)
    {
       /* for (int i = 0; i < _brojRedova; i++)
           ...
           return nullptr;*/
                ...
                if (&_sjedista[red][kolona] == nullptr)
                    return nullptr;
...
public:
    ...
    void Ispis()
    {
        ...
        cout << "Spol: ";  *_spol ? cout << "Musko" : cout << "Zensko";
        cout << endl;
...
public:
    ...
    int GetMaxBrojDvorana() const { return *_maxBrojDvorana;  } //* zato sto je int* _maxBrojDvorana
    int GetTrenutnoDvorana() const { return _trenutnoDvorana;  } //bez * zato sto nema int* _trenutnoDvorana
    ...
    void Ispis()
    {
        ...
        for (int i = 0; i < _trenutnoDvorana; i++)
            cout << _dvorane[i].GetNaziv() << endl; //nema -> jer smo imali samo _dvorane, pokazivac na niz
        cout << "*******************UPOSLENICI*******************\n";
        for (int i = 0; i < _trenutnoUposlenika; i++)
            cout << _uposlenici[i]->GetIme() << " " << _uposlenici[i]->GetPrezime() << endl; // -> zato sto smo znali vec da je _uposlenici[50]; niz pokazivaca
    }
    //Z4.8
    ~Kino() {
        delete[] _naziv;
...
void Zadatak4() {
    ...
    Dvorana d1("Dvorana 1", 10, 5);
    Dvorana d2("Dvorana 2", 10, 6);
